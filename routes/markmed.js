...
// Tahtlikult ainult admin: siia võib panna ka asju, mis pole töötajatele mõeldud.
...
// Tehtud märkmed vajuvad omaette alla, et nimekiri ei kasvaks üle pea.
...
router.put('/:id', noudaAdmin, async (req, res) => {
  ...
  try {
    const r = await pool.query(
      'UPDATE admin_markmed SET tekst=$1, uuendatud=NOW() WHERE id=$2 RETURNING *',
      [tekst, req.params.id]
    );
...
router.put('/:id/tehtud', noudaAdmin, async (req, res) => {
  try {
    const r = await pool.query(
      'UPDATE admin_markmed SET tehtud=$1, uuendatud=NOW() WHERE id=$2',
      [!!req.body.tehtud, req.params.id]
    );
    if (!r.rowCount) return res.json({ ok: false, veateade: 'Märget ei leitud' });
    res.json({ ok: true });
  } catch (err) {
